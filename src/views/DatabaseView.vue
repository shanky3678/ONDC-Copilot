            ...
            <div class="w-full border border-[#DFDFDF] pt-[55px] px-[55px] pb-8 rounded-lg bg-white">
                ...
                <div class="mt-6">
                    ...
                    <table class=" table-auto w-full shadow-lg rounded-bl-lg rounded-br-lg">
                        ...
                        <tbody v-if="searchResult.length > 0" v-for="document of searchResult" style="overflow-y: auto;">
                            <tr class="w-full border-b border-[#E1CFFF] hover:bg-[#E1CFFF] bg-white">
                                <td class="min-w-[250px] p-6 flex items-center">
  ...
  <div
    ...
    <div
      ...
      <div class="relative">
        ...
        <div v-show="document_category == 'Other'" class="w-full mt-[20px]">
                    <label for="" class="font-bold text-[#1F263E] text-base ml-[22px]">Category </label>
                    <input id="1" type="text" v-model="new_category" class="w-full mt-[14px] text-xs font-medium text-[#0C0C0C] border-[#8AC0FF] outline-0 rounded-[10px] focus:border-[#8AC0FF] border py-[22px] px-[45px] placeholder:text-[#979797] placeholder:font-normal placeholder:text-xs" placeholder="Enter the category">
                </div>
            </div>
            ...
            <div class="w-full mt-9 ">
                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="file_input">Upload law file</label>
            <input @change="handleFileChange" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" aria-describedby="file_input_help" id="file_input" type="file" ref="fileInput" accept=".pdf">
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-300" id="file_input_help">Supported formats: PDF</p>
            </div>

              
            <div class="mt-6 flex justify-center w-full mx-auto">
                <button id="lawFormData" type="button" data-te-toggle="modal" data-te-modal-dismiss aria-label="Close" 
                data-te-ripple-init data-te-ripple-color="light" class="bg-[#5D81F3] w-full rounded px-[65px] py-[18px]" ><h1 class="text-white text-xs font-normal" @click="submit" >SUBMIT</h1></button>
            </div>
        </div>
      </div>
...
const searchResult = computed(() => {
    return documents.value.filter(document => document.document_name.toLowerCase().includes(searchText.value.toLowerCase()))
})
...
const categories = ['All','Fashion', 'Food & beverage', 'Electronic','Agriculture', 'Other'];
...
async function submit(){
    ...
    if(!isEmptyOrNull(document_name.value) && (!isEmptyOrNull(document_category.value) || !isEmptyOrNull(new_category.value)) && !isEmptyOrNull(document_source_url.value) && !isEmptyOrNull(document_remark.value) && !isEmptyOrNull(file.value)){
        let response = await server.uploadDocument(server.userDetails.user_id, document_name.value, document_category.value == "Other" ? new_category.value : document_category.value , document_source_url.value, document_remark.value, file.value.files[0])
